...
import time
import os
os.path.abspath("/var/webvapt/reports/vapt/")
...
def vapt_scan():
    ...
    """  
        ...
        vapt_output = input(conf.colored(f"report path - [default: /var/webvapt/reports/vapt/{vapt_host}]: ", "green", attrs=["bold"], ))
        conf.not_valid(vapt_scan, vapt_host)
        vapt_output = conf.dir_output(vapt_output, "/var/webvapt/reports/vapt/", vapt_host)
        ...
        if len(vapt_host) == 0:
            ...
            conf.os.system(f"wapiti --flush-attack --flush-session  -u http://{vapt_host} -v 2 -dr -f html -o {vapt_output}/wapiti")
            ...
            conf.os.system(f"commix -u http://{vapt_host} --level 2 --all -v 2 --batch --output-dir=/var/webvapt/reports/vapt/{vapt_host}")
            conf.os.system(f"python3 /var/webvapt/modules/msf/msf.py -j 6 -t {vapt_host}  -f {vapt_output}")
            ...
            print("\n")
            #end time
            print(conf.colored("\n Time used: ", "white", attrs=["reverse"]) + conf.colored(f" {time.time() - startTime}          Segundos ", "yellow", attrs=["reverse"]))
